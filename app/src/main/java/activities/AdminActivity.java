...
public class AdminActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			LocationAccelerationDao lDB = new LocationAccelerationDao(Constants.databaseName, Constants.locationTable, this);
			String allLocs = lDB.getAllLocationsFromDatabase().size()+"";
            String toUploadLocs = lDB.getLocationsForUploadFromDatabase(-1).size() + "";
            lDB.closeDb();
...
*/		databaseStatusBody.setText(databaseBodyString);
		...
		final AdministrativeDao adminDb = new AdministrativeDao(
		...
		testConnection.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					new CheckURLTask(urlText.getText().toString(),
        ...
        speedText.setText(String.valueOf(adminDb.getSpeedThresh()));
		...
		saveAndExit.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				startActivity(new Intent(AdminActivity.this, ServiceHandlingActivity.class));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		item.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				Intent aboutIntent = new Intent(AdminActivity.this, AboutActivity.class);
				try {
